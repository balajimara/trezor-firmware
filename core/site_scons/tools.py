...
def embed_binary(obj_program, env, section, target_, file):
    ...
    end_src = (
        "_binary_build_firmware_" + _in.replace("/", "_").replace(".", "_") + "_end"
    )
    size_src = (
        "_binary_build_firmware_" + _in.replace("/", "_").replace(".", "_") + "_size"
    )
    start_dest = (
        "_binary_" + target_.replace("/", "_").replace(".o", "_bin_deflated") + "_start"
    )
    end_dest = (
        "_binary_" + target_.replace("/", "_").replace(".o", "_bin_deflated") + "_end"
    )
    size_dest = (
        "_binary_" + target_.replace("/", "_").replace(".o", "_bin_deflated") + "_size"
    )
